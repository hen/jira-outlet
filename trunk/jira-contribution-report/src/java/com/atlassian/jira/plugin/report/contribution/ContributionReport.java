...
public class ContributionReport extends AbstractReport {
    ...
    public String generateReportHtml(ProjectActionSupport action, Map params) throws Exception {
        ...
        builder = builder.project(project.getId());
        if(version != null) {
            builder = builder.and().fixVersion(version.getId());
        }
        Query query = builder.buildQuery();
        SearchResults searchResults = searchProvider.search(query, remoteUser, PagerFilter.getUnlimitedFilter());
        ...
        if("open".equals(issueStatus)) {
            velocityParams.put("statusFilter", "&resolution=-1");
        } else 
        if("closed".equals(issueStatus)) {
            velocityParams.put("statusFilter", "&status=5&status=6");
        }
